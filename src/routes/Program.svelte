<script lang="ts">
  ...
  let selectedWeek = $state<WeekNumber>(9)
  let openDay = $state<number>(0) // index of expanded day within the week (-1 = none)
...
<div class="days">
  {#each days as day, i (i)}
    <DayCard {day} open={openDay === i} onToggle={() => toggleDay(i)} />
  {/each}
</div>
